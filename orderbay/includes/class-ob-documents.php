...
/**
 * A) Invoice + packing slip (HTML print-friendly).
 */
class OB_Documents {
	...
	public function render_settings() {
		...
		echo '<tr><th><label for="ob_from">' . esc_html__( 'From address', 'orderbay' ) . '</label></th><td>';
		...
		echo '</form>';
		echo '<p class="description">' . esc_html__( 'Print sheets are HTML (browser Print → PDF). No external SaaS. Independent of Twilio Order Communicator.', 'orderbay' ) . '</p>';
		echo '</div>';
	}
	...
	public function action_print_invoice( $order ) {
		// Redirect handled poorly for order actions; button uses admin-post.
		$url = $this->print_url( $order->get_id(), 'invoice' );
		$order->add_order_note( sprintf( __( 'Orderbay invoice link: %s', 'orderbay' ), $url ), false, true );
	}

	public function action_print_packing( $order ) {
		$url = $this->print_url( $order->get_id(), 'packing' );
		$order->add_order_note( sprintf( __( 'Orderbay packing slip link: %s', 'orderbay' ), $url ), false, true );
	}
	...
	public function bulk_actions_legacy( $actions ) {
		$actions['ob_print_invoices'] = __( 'Orderbay: print invoices', 'orderbay' );
		$actions['ob_print_packing']  = __( 'Orderbay: print packing slips', 'orderbay' );
		return $actions;
	}
	...
	private function handle_bulk( $redirect, $action, $ids ) {
		...
		$url  = wp_nonce_url(
			admin_url( 'admin-post.php?action=ob_bulk_print&type=' . $type . '&ids=' . implode( ',', $ids ) ),
			'ob_bulk_print'
		);
	...
	public function handle_bulk_print() {
		...
		$ids  = isset( $_GET['ids'] ) ? array_map( 'absint', explode( ',', sanitize_text_field( wp_unslash( $_GET['ids'] ) ) ) ) : array(); // phpcs:ignore
		$orders = array();
		foreach ( $ids as $id ) {
			$o = wc_get_order( $id );
			if ( $o ) {
