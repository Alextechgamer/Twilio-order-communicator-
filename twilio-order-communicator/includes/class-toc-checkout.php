...
class TOC_Checkout {
	...
	public function sync_classic_after_process( $order_id, $posted = array(), $order = null ) {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Missing
		if ( ! isset( $_POST['toc_sms_consent'] ) && empty( $_POST ) ) {
			return;
		}
		// phpcs:ignore WordPress.Security.NonceVerification.Missing
		if ( array_key_exists( 'toc_sms_consent', $_POST ) || ! empty( $_POST['billing_first_name'] ) || ! empty( $_POST['woocommerce-process-checkout-nonce'] ) ) {
			// phpcs:ignore WordPress.Security.NonceVerification.Missing
			$consented = ! empty( $_POST['toc_sms_consent'] );
			// Classic unchecked is explicit no only when we know this was a classic checkout post.
			// phpcs:ignore WordPress.Security.NonceVerification.Missing
			$explicit = array_key_exists( 'toc_sms_consent', $_POST ) || ! empty( $_POST['woocommerce-process-checkout-nonce'] );
	...
	private function read_block_consent_state( $order, $request = null ) {
		...
		if ( is_object( $request ) && method_exists( $request, 'get_param' ) ) {
			...
			foreach ( $bags as $bag ) {
				...
				foreach ( array( self::BLOCK_FIELD_ID, 'toc/sms-consent', 'toc-sms-consent', 'sms-consent' ) as $key ) {
	...
	private function order_block_consent_state( $order ) {
		...
		foreach ( $order->get_meta_data() as $meta ) {
			...
			if ( in_array( $key, array( '_toc_sms_consent', self::meta_key() ), true ) ) {
				continue;
			}
			$val = $data['value'] ?? '';
			if ( $twilio->is_truthy_consent( $val ) ) {
