...
class ORL_Logger {
	...
	/**
	 * Scalable STOP / opt-out list (replaces serialized orl_sms_opt_outs option).
	 */
	...
	public function find_order_by_phone( $phone ) {
		...
		if ( class_exists( 'ORL_Statuses' ) ) {
			...
			$statuses[] = ORL_Statuses::bare_status( ORL_Statuses::mapped_shipped_status() );
		}
	...
	public function get_bulk_pickup_orders( $args = array() ) {
		...
		$status = ORL_Statuses::bare_status( ORL_Statuses::mapped_ready_status() );
		...
		foreach ( $orders as $order ) {
			if ( class_exists( 'ORL_Order_Meta' ) && ORL_Order_Meta::is_collected( $order ) ) {
				continue;
			}
			if ( (int) get_option( 'orl_ready_require_local_pickup', 0 ) === 1 && ! ORL_Auto::is_local_pickup( $order ) ) {
				continue;
			}
			...
			if ( $cutoff > 0 ) {
				$last = $order->get_meta( '_orl_last_reminder_at' );
				if ( $last ) {
